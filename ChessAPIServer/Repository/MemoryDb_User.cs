...
public partial class MemoryDb : IMemoryDb
{
    ...
    /// </summary>
    public async Task<ErrorCode> RegisterUserAuthAsync(string login_token, long uid)
    {
        ...
        try
        {
            ...
            {
                _logger.ZLogError($"[RegistUserAsync] Uid:{uid}, LoginToken:{login_token}, ErrorMemssage : UserBasicAuth, RedisString set Error");
        ...
        {
            _logger.ZLogError(e, $"[RegistUserAsync] Uid:{uid}, LoginToken:{login_token}, ErrorMessage:Redis Connection Error");
            result = ErrorCode.LoginFailAddRedis;
            return result;
        }
